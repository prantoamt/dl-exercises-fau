...
class TestGen(unittest.TestCase):
    def setUp(self):
        ...
        self.label_path = "./ex_0/src_to_implement_pranto/Labels.json"
        self.file_path = "./ex_0/src_to_implement_pranto/exercise_data/"
    ...
    def testShuffle(self):
        ...
        np.testing.assert_raises(
            AssertionError, np.testing.assert_array_equal, gen.next()[0], gen2.next()[0]
        )
        np.testing.assert_raises(
            AssertionError, np.testing.assert_array_equal, gen.next()[1], gen2.next()[1]
        )
    ...
    def testShuffleEpoch(self):
        ...
        self.assertFalse(
            np.all(np.sort(b1_epoch0, axis=None) == np.sort(b1_epoch1, axis=None))
        )
    ...
    def testRotation(self):
        ...
        augmented_images_indices = np.sum(
            np.abs(batch1 - batch2), axis=(1, 2, 3)
        ).astype(np.bool_)
        ...
        rot3 = np.sum(
            original_corners[:, ::-1, 1] - characteristic_corners, axis=(1, 2)
        )
        ...
        np.testing.assert_raises(
            AssertionError, np.testing.assert_array_equal, batch1, batch2
        )
    ...
    def testMirroring(self):
        ...
        horizontal_vertical = np.sum(
            original_corner_diagonals - vertical_horizontal_augmented_corners,
            axis=(1, 2),
        )
