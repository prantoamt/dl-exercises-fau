...
class ImageGenerator:
    ...
    def __init__(self, file_path, label_path, batch_size, image_size, rotation=False, mirroring=False, shuffle=False):
        ...
        self.all_images = [file for file in os.listdir(self.file_path) if os.path.isfile(os.path.join(self.file_path, file))]
        if self.shuffle:
        ...
        self.dataset_pointer    = 0     # for tracking the current position of the images array in each epoch
    ...
    def next(self):
        ...
        if self.dataset_pointer+self.batch_size <= len(self.all_images):
            ...
            images_list = self.all_images[self.dataset_pointer:self.dataset_pointer+self.batch_size]
            ...
            img_arr1    = self.all_images[self.dataset_pointer:]    # if the dataset pointer reach into the end of all image array then retrieve remaining images
            data_need   = self.batch_size - (len(self.all_images) - self.dataset_pointer)   # calculate how much more data we need in last batch
            img_arr2    = self.all_images[:data_need]   # retrieve the calculated data from the begining of the images array
            images_list = img_arr1 + img_arr2   # merge both of the array to create the last batch
        ...
        for img in images_list:
            ...
            with open(os.path.join(self.file_path, img), 'rb') as f:
                img = np.load(f)
                img = self.resize_image(img)    # resize according to their desire dimensions

                if self.mirroring or self.rotation:
                    img = self.augment(img) # augment data if the mirroring and rotation parameters are true
