...
class FullyConnected(BaseLayer):
    ...
    def backward(self, error_tensor: np.ndarray) -> np.ndarray:
        new_error_tensor = np.dot(error_tensor, self.weights.T)  # calculate the error tensor to pass in the previous layer
        self._gradient_weights = np.dot(self.input_tensor.T, error_tensor)  # calculate gradient to update the weights using optimizer
