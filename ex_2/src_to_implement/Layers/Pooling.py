            ...
            for channel_ind in range(channel):
                ...
                for start_row in range(0, image_row, stride_row):
                    ...
                    for start_col in range(0, image_col, stride_col):
                        ...
                        max_pool_index = (
                            row_index[0] + qaurter_ind_row,
                            col_index[0] + qaurter_ind_col,
                        )
                ...
                max_pooling_2d_img = np.array(max_pooling_2d_img).reshape(
                    pool_row, pool_col
                )
            ...
            for channel_ind in range(channel):
                ...
                for img_err_rows in image_err_2d:
                    for img_err_val in img_err_rows:
                        target_row, target_col = self.output_tensor_max_ind_list[
                            output_tensor_max_ind_counter
                        ]

                        if (
                            output_error_tensor[
                                batch_ind, channel_ind, target_row, target_col
                            ]
                            == 0
                        ):
                            output_error_tensor[
