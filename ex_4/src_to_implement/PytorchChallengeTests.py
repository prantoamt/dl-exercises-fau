        ...
        try:
            ...
            if match:
                ...
                if bool(os.stat(path).st_mode & (stat.S_IRWXO | stat.S_IRWXG)):
                    ...
                    # fail with a message
                    self.fail(
                        "I fixed the permissions of your PyCharm remote interpreter directory for you."
                    )
...
class TestDataset(unittest.TestCase):
    ...
    def test_shape(self):
        ...
        val_dl = t.utils.data.DataLoader(
            ChallengeDataset(self.tab, "val"), batch_size=1
        )
        for x, y in val_dl:
            x = x[0].cpu().numpy()
            self.assertEqual(
                x.shape[0],
                3,
                'Make sure that your images are converted to RGB. On the "__getitem__" function use the respective skimage function (for further information check the Description.pdf file)',
            )
...
class TestModel(unittest.TestCase):
    ...
    def test_prediction(self):
        ...
        self.assertFalse(
            np.isnan(pred).any(),
            "Your prediction contains NaN values \n Possible reasons: \n - Wrong Loss function \n - Poorly choosen learning rate \n - Faulty input",
        )
        self.assertFalse(
            np.isinf(pred).any(),
            "Your prediction contains inf values \n Possible reasons: \n - Wrong Loss function \n - Poorly choosen learning rate \n - Faulty input",
        )
        self.assertTrue(
            np.all([0 <= pred, pred <= 1]),
            "Make sure your predictions are sigmoided i.e the last layer of your ResNet model should be the sigmoid activation function",
        )
    ...
    def test_prediction_after_save_and_load(self):
        ...
        ort_inputs = {
            ort_session.get_inputs()[0].name: t.rand((50, 3, 300, 300)).numpy()
        }
        ...
        self.assertTrue(
            np.all([0 <= pred, pred <= 1]),
            "Make sure your predictions are sigmoided i.e the last layer of your ResNet model should be sigmoid activation function",
        )
    ...
    if sys.argv[-1] == "Bonus":
        ...
        for test, p in zip(tests, percentages):
            if (
                ...
                .run(loader.loadTestsFromTestCase(test))
                .wasSuccessful()
            ):
        ...
        print(
            tabulate.tabulate(
                table,
                headers=["Pos", "Test", "Result", "Percent", "Percent in Exam"],
                tablefmt="github",
            )
        )
    else:
